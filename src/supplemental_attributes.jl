...
function _attach_attribute!(
    ...
    is_attached(mgr, attribute) && return
...
function is_attached(mgr::SupplementalAttributeManager, attribute::SupplementalAttribute)
    T = typeof(attribute)
...
function remove_supplemental_attribute!(
    attributes::SupplementalAttributeManager,
    component::InfrastructureSystemsComponent,
    attribute::SupplementalAttribute,
)
    remove_association!(attributes.associations, component, attribute)
    if !has_association(attributes.associations, attribute)
        remove_supplemental_attribute!(attributes, attribute)
    end
end
...
function remove_supplemental_attribute!(
    ...
    if !force && has_association(mgr.associations, supplemental_attribute)
        throw(
            ArgumentError(
