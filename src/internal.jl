...
mutable struct ComponentComposition <: InfrastructureSystemsType
    ...
    is_composed::Bool
    "true if the component has been removed from the system but still contains internal references to time series"
    is_foreign::Bool
end

function ComponentComposition(; is_composed = false, is_foreign = false)
    return ComponentComposition(is_composed, is_foreign)
end
...
set_foreign!(internal::InfrastructureSystemsInternal) =
    internal.composition.is_foreign = true
is_foreign(internal::InfrastructureSystemsInternal) = internal.composition.is_foreign
