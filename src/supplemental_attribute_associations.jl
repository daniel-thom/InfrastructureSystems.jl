...
"""
Add a supplemental attribute association to the associations.
"""
function add_association!(
    ...
    if has_association(associations, component, attribute)
        throw(
            ArgumentError(
                "An association between $(summary(attribute)) and $(summary(component)) is already stored",
            ),
        )
    end
    row = (
...
"""
Clear all supplemental attribute associations.
"""
function clear_associations!(associations::SupplementalAttributeAssociations)
    _execute(associations, "DELETE FROM $SUPPLEMENTAL_ATTRIBUTE_TABLE_NAME")
end
...
"""
Remove all associations containing the supplemental attribute.
"""
function remove_associations!(
    associations::SupplementalAttributeAssociations,
    attribute::SupplementalAttribute,
    ...
    where_clause = "WHERE attribute_uuid = '$uuid'"
    num_deleted = _remove_associations!(associations, where_clause)
    @debug "Deleted $num_deleted supplemental attribute associations" _group =
        LOG_GROUP_SUPPLEMENTAL_ATTRIBUTES
    return
...
"""
Remove all associations containing the component.
"""
function remove_associations!(
    associations::SupplementalAttributeAssociations,
    component::InfrastructureSystemsComponent,
    ...
    c_uuid = get_uuid(component)
    where_clause = "WHERE component_uuid = '$c_uuid'"
    num_deleted = _remove_associations!(associations, where_clause)
    @debug "Deleted $num_deleted supplemental attribute associations" _group =
        LOG_GROUP_SUPPLEMENTAL_ATTRIBUTES
    return
...
function from_records!(associations::SupplementalAttributeAssociations, records)
    ...
    # SQLite/DBInterface seems to want this data by column instead of by row,
    # which seems unusual for SQLite.
